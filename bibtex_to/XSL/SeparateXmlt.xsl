...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    <xsl:param name="xslOutput"></xsl:param>
    ...
    <xsl:template match="bib:entry">
        <xslt>
            <xsl:attribute name="xslresource"><xsl:value-of select="$xslFile"/></xsl:attribute>
            <xsl:attribute name="in"><xsl:value-of select="$in"/></xsl:attribute>
            <xsl:attribute name="out"><xsl:value-of select="$out"/>/<xsl:value-of select="./@id"/>.<xsl:value-of select="$xslOutput"/></xsl:attribute>
            <param name="Position">
